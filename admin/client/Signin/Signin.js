 ...
 */
 ...
 var ADMIN_LOGIN_GOOGLE_RECAPTCHA = true
 
 var recaptchatToken = '';
 
 var verifyCallback = recaptchatCallbackToken => {
	 recaptchatToken = recaptchatCallbackToken
 }
 ...
 var SigninView = React.createClass({
	 ...
	 componentDidMount () {
		 ...
		 loadReCaptcha('6LdGkLEUAAAAAFksIYjOwbhzqRCriqy8YubIDqYD')
	 },
	 handleInputChange (e) {
	 ...
	 handleSubmit (e) {
		 e.preventDefault();
		 // google recaptchat
		 if(!recaptchatToken && ADMIN_LOGIN_GOOGLE_RECAPTCHA === true){
			 return this.displayError('Login is abnormal, please try again later.');
		 }
		 ...
		 xhr({
			 ...
			 json: {
				 ...
				 recaptchatToken: recaptchatToken
			 },
			 headers: assign({}, Keystone.csrf.header),
		 }, (err, resp, body) => {
			 this.recaptcha.execute()
			 if (err || body && body.error) {
				 if (body.error === 'Login is abnormal, please try again later.') {
		 ...
		 return (
			 <div className="auth-wrapper">
				 <ReCaptcha
					 ...
					 action="adminLogin"
					 sitekey='6LdGkLEUAAAAAFksIYjOwbhzqRCriqy8YubIDqYD'
				 ...
				 <div className="auth-footer">
					 ...
					 <a href="javascript:;" target="_blank" title="The Node.js CMS and web application platform (new window)">Skojalab</a>
				 </div>
			 </div>
		 );
	 },
